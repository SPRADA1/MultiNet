...
multinet<-defmacro(all.dat_ord_global ,medi_global,var_s1, diff_global1,
                   medi_global_control,var_s1_control,medi_global_case,
                   var_s1_case, expr = {
...
hist1<-ggplot(df, aes(x=nodes, color=sample, fill=sample)) + 
  ...
  geom_density(alpha=.2) +xlab("Median methylation")+
...
hist2<-ggplot(df, aes(x=nodes, color=sample, fill=sample)) + 
  geom_histogram(aes(y=..density..), alpha=0.5, position="identity")+
  geom_density(alpha=.2) +xlab("Median difference methylation")+
  theme(text = element_text(size=20))
...
hist3<-ggplot(df, aes(x=nodes, color=sample, fill=sample)) + 
  geom_histogram(aes(y=..density..), alpha=0.5, position="identity")+
  geom_density(alpha=.2) +xlab("Median correlation")+
  theme(text = element_text(size=20))
...
hist4<-ggplot(df, aes(x=nodes, color=sample, fill=sample)) + 
  geom_histogram(aes(y=..density..), alpha=0.5, position="identity")+
  geom_density(alpha=.2) +xlab("Variance")+
  theme(text = element_text(size=20))
...
reset_par()
par(mfrow=c(2,1))
barcor<-bargen(dms_global_sig,all.dat_ord_global,20)
dev.off()
...
dms_genes_global_cg<-bargen(dms_global_sig,all.dat_ord_global,0)[[3]]
